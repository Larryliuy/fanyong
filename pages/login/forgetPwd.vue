<template>
	<page-meta>
		<navigation-bar :title="$t('v_qisvimima')" :front-color="$colors.white" :background-color="$colors.mainColor"/>
	</page-meta>
	<view class="page-box column-center">
		<form @submit="resetPwd">
<!--			<view class="input-box flex">-->
<!--				<input name="tel" type="number" :placeholder="$t('v_quurubjihk')"/>-->
<!--			</view>-->
			...
			<view class="input-box flex">
				<input name="pwd" password :placeholder="$t('v_quuruxnmima')"/>
			</view>
			<view class="row-center">
				<button class="xf-btn" :style="'background-color:'+$colors.button2" form-type="submit">{{$t('v_qisvimima')}}</button>
			</view>
		</form>
		...
		methods: {
			resetPwd(e) {
				...
				if (this.environment === 'me_pro') {
					...
					// val.invite_code = val.invite_code
				} else {
					if (!val.verify) {
					  return this.openShowToast(this.$t('v_qjdxnsiwjig'))
					}
